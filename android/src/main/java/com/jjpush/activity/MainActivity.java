...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.d("jjpush", " 回调= " + url);
        Map map = getUrlParams(url);
        Log.d("jjpush", " 自定义参数= " + map);
        Gson gson = new Gson();
        RNJJPushPackage.reactContext.getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter.class).emit("jjpush_click", gson.toJson(map));
        finish();
    }
     ...
     * @return 参数字符串
     */
    public String getUrlParamStr(String url) {
        URL mUrl = stringToURL(url);
        if (mUrl == null) {
            return "";
        }
     ...
     * @return 参数值
     */
    public String getUriParam(Uri uri, String paramKey) {
     ...
     * @return 参数值
     */
    public int getIntUriParam(Uri uri, String paramKey) {
        if (uri == null || paramKey == null || paramKey.length() == 0) {
            return 0;
        }
        try {
